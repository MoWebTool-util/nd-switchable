...
describe('Switchable', function() {
  ...
  function testTriggerAndPanelActive(sw, active) {
    ...
    for (var i = 0, len = sw.length; i < len; i++) {
      ...
      if (active === i) {
        ...
        expect(panel.css('display')).to.equal('block')
      } else {
  ...
  it('Switchable初始化', function() {
    ...
    expect(switchable instanceof Switchable).to.equal(true)
    expect(switchable.get('triggers').length).to.equal(3)
    expect(switchable.get('panels').length).to.equal(3)
  ...
  it('Switchable 属性配置', function() {
    ...
    expect(switchable.get('triggerType')).to.equal('click')
    expect(switchable.get('activeIndex')).to.equal(1)
  })
  ...
  it('triggers自动初始化', function() {
    ...
    expect(switchable.get('triggers').length).to.equal(3)
    expect(switchable.get('panels').length).to.equal(3)
    expect($(switchable.get('triggers')[0]).html()).to.equal('1')
    expect($(switchable.get('triggers')[1]).html()).to.equal('2')
  })
  ...
  it('Carousel', function() {
    ...
    expect(cc.get('circular')).to.equal(true)
    cc.switchTo(2)
  ...
  it('Accordion', function() {
    ...
    expect(ac.get('multiple')).to.equal(false)
  ...
  it('插件安装 一', function() {
    ...
    expect(!!sw.get('autoplay')).to.equal(false)
    expect(!!sw.get('circular')).to.equal(false)
    expect(sw.get('effect')).to.equal('none')
    expect(sw._plugins.length).to.equal(0)
    ...
    expect(slide.get('autoplay')).to.equal(true)
    expect(slide.get('circular')).to.equal(true)
    expect(slide._plugins.length).to.equal(3)
    ...
    expect(sw2._plugins.length).to.equal(1)
  })
  ...
  it('插件安装 二', function() {
    ...
    expect(sw._plugins.length).to.equal(2)
  })
  ...
  it('destroy', function() {
    ...
    sw.destroy()
    expect(sw._plugins).to.equal(undefined)
  })
